# ChromaDesk Signaling Server Deployment Guide
...
```bash
git clone git@github.com:barry-ran/ChromaDesk.git
cd ChromaDesk/SignalingServer
...
| `DB_USER` | chromadesk | Database user |
| `DB_PASSWORD` | chromadesk123 | Database password |
| `DB_NAME` | chromadesk_signaling | Database name |
...
3. **Configure security**:
   - **API Key**: Admin Panel → Settings → API Key. When set, only clients carrying this key can connect to the signaling server, preventing unauthorized client access. Native clients (ChromaDesk desktop) can configure the API Key in **Settings → Network → API Key** at runtime — no recompilation needed
   - **Allowed Origins**: Admin Panel → Settings → Allowed Origins. When the WebClient is deployed on a separate domain (e.g. `https://web.chromadesk.cc`), browsers block cross-origin requests by default. Add the WebClient's domain here so the signaling server allows CORS requests from those origins. Separate multiple domains with commas
4. **Configure WebClient URL** (optional): Admin Panel → Preset → WebClient URL. Enter the WebClient deployment address (e.g. `https://web.chromadesk.cc`). The native client will display this link in its UI so users can quickly access the WebClient
...
sudo mkdir -p /data/chromadesk/{postgres,redis}

# Start PostgreSQL
docker run -d --name chromadesk-postgres \
  --restart=always \
  -p 5432:5432 \
  -e POSTGRES_USER=chromadesk \
  -e POSTGRES_PASSWORD=chromadesk123 \
  -e POSTGRES_DB=chromadesk \
  -v /data/chromadesk/postgres:/var/lib/postgresql/data \
  postgres:15

# Start Redis
docker run -d --name chromadesk-redis \
  --restart=always \
  -p 6379:6379 \
  -v /data/chromadesk/redis:/data \
...
CGO_ENABLED=0 GOOS=linux GOARCH=amd64 \
  ...
  -o chromadesk_signaling ./cmd/signaling

# Create runtime directory and deploy
sudo mkdir -p /opt/chromadesk-signaling
sudo cp chromadesk_signaling /opt/chromadesk-signaling/
```
...
sudo cp .env /opt/chromadesk-signaling/.env

# Edit configuration as needed (database, TURN, API key, etc.)
sudo vim /opt/chromadesk-signaling/.env

# Create systemd service
sudo cat > /etc/systemd/system/chromadesk-signaling.service << 'EOF'
[Unit]
Description=ChromaDesk Signaling Server
...
Type=simple
User=root
WorkingDirectory=/opt/chromadesk-signaling
ExecStart=/opt/chromadesk-signaling/chromadesk_signaling
Restart=always
RestartSec=5
...
sudo systemctl start chromadesk-signaling
sudo systemctl enable chromadesk-signaling

# Check service status
sudo systemctl status chromadesk-signaling
```
...
/opt/chromadesk-signaling/chromadesk_signaling --env /etc/chromadesk/production.env
```
...
```bash
sudo vim /opt/chromadesk-signaling/.env
sudo systemctl restart chromadesk-signaling
```
...
sudo cat > /etc/nginx/conf.d/chromadesk.conf << 'EOF'
...
sudo journalctl -u chromadesk-signaling -f
...
docker logs -f chromadesk-postgres
docker logs -f chromadesk-redis
```
...
# Stop service
sudo systemctl stop chromadesk-signaling

# Access database
docker exec -it chromadesk-postgres psql -U chromadesk -d chromadesk

# Query device count
docker exec -it chromadesk-postgres psql -U chromadesk -d chromadesk -c "SELECT COUNT(*) FROM devices;"

# Backup database
docker exec chromadesk-postgres pg_dump -U chromadesk chromadesk > /backup/chromadesk_$(date +%Y%m%d).sql

# Restore database
cat backup.sql | docker exec -i chromadesk-postgres psql -U chromadesk -d chromadesk
...
```bash
# View detailed logs
sudo journalctl -u chromadesk-signaling -n 100 --no-pager

# Check port usage
sudo lsof -i:8000

# Check database connectivity
docker exec -it chromadesk-postgres psql -U chromadesk -d chromadesk -c "SELECT 1;"
docker exec -it chromadesk-redis redis-cli ping
```
...
docker restart chromadesk-postgres chromadesk-redis

# Check database logs
docker logs chromadesk-postgres
```
...
# Stop container
docker stop chromadesk-postgres
docker rm chromadesk-postgres
