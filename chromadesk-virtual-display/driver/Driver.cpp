...
//
// Define an Interface Guid for ChromaDesk Virtual Display device class.
// This GUID is used to register (IoRegisterDeviceInterface)
// an instance of an interface so that user application
// can control the ChromaDesk Virtual Display device.
//
...
void IndirectDeviceContext::InitAdapter()
{
    ...
    AdapterCaps.EndPointDiagnostics.pEndPointFriendlyName = L"ChromaDesk Virtual Display";
    AdapterCaps.EndPointDiagnostics.pEndPointManufacturerName = L"QuickCoder";
    AdapterCaps.EndPointDiagnostics.pEndPointModelName = L"ChromaDesk Virtual Display";
...
IddRustDeskIoDeviceControl(WDFDEVICE Device, WDFREQUEST Request, size_t OutputBufferLength, size_t InputBufferLength, ULONG IoControlCode)
{
    ...
    {
    case IOCTL_CHROMADESK_VD_PLUG_IN:
        PChromaDeskPlugInParams pPlugIn;
        Status = WdfRequestRetrieveInputBuffer(Request, sizeof(ChromaDeskPlugInParams), &Buffer, &BufSize);
        if (!NT_SUCCESS(Status)) {
            TraceEvents(TRACE_LEVEL_ERROR,
                TRACE_DEVICE,
        ...
        {
            // Adapt ChromaDesk params to internal CtlPlugIn
            pPlugIn = (PChromaDeskPlugInParams)Buffer;
            CtlPlugIn plugIn;
            plugIn.ConnectorIndex = pPlugIn->ConnectorIndex;
            plugIn.MonitorEDID = pPlugIn->MonitorEDID;
            plugIn.ContainerId = pPlugIn->ContainerId;
            Status = pContext->pContext->PlugInMonitor(&plugIn);
        }
        break;
    case IOCTL_CHROMADESK_VD_PLUG_OUT:
        PChromaDeskPlugOutParams pPlugOut;
        Status = WdfRequestRetrieveInputBuffer(Request, sizeof(ChromaDeskPlugOutParams), &Buffer, &BufSize);
        if (!NT_SUCCESS(Status))
        {
            TraceEvents(TRACE_LEVEL_ERROR,
                TRACE_DEVICE,
        ...
        {
            pPlugOut = (PChromaDeskPlugOutParams)Buffer;
            CtlPlugOut plugOut;
            plugOut.ConnectorIndex = pPlugOut->ConnectorIndex;
            Status = pContext->pContext->PlugOutMonitor(&plugOut);
        }
        break;
    case IOCTL_CHROMADESK_VD_UPDATE_MODE:
        PChromaDeskUpdateModeParams pUpdateMode;
        ...
        {
            pUpdateMode = (PChromaDeskUpdateModeParams)Buffer;
            // Reinterpret as CtlMonitorModes (binary compatible layout)
            PCtlMonitorModes pModes = (PCtlMonitorModes)Buffer;
            Status = pContext->pContext->UpdateMonitorModes(pModes);
        }
        break;
    case IOCTL_CHROMADESK_VD_QUERY:
        {
            // Return active monitor status via output buffer
            PVOID OutBuffer;
            size_t OutBufSize;
            Status = WdfRequestRetrieveOutputBuffer(Request, sizeof(ChromaDeskQueryResult), &OutBuffer, &OutBufSize);
            if (NT_SUCCESS(Status))
            {
                PChromaDeskQueryResult pResult = (PChromaDeskQueryResult)OutBuffer;
                memset(pResult, 0, sizeof(ChromaDeskQueryResult));
                pResult->ActiveCount = 0;
                for (UINT i = 0; i < CHROMADESK_VD_MAX_MONITORS && i < pContext->pContext->GetMaxMonitorCount(); i++)
                {
                    pResult->Monitors[i].ConnectorIndex = i;
                    // Check if monitor is active by trying to access the monitor array
                    // (simplified - in production, expose a proper query method)
                    pResult->Monitors[i].Active = FALSE; // Will be set by enhanced CheckIndex
                }
                WdfRequestSetInformation(Request, sizeof(ChromaDeskQueryResult));
            }
        }
        break;
    default:
