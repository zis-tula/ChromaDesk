...
export class Session extends EventTarget {
    ...
    async connect(deviceId, accessCode) {
        ...
        try {
            ...
            // 2. 构建正确的 JID 格式
            // 参照 C++ ChromaDeskClient:
            //   client local_id = "client_{client_device_id}@chromadesk.local/chromoting_ftl_chromadesk_client"
            //   remote_id = "{host_id}@chromadesk.local/chromoting_ftl_chromadesk_host"
            const clientUUID = generateUUID().replace(/-/g, '').substring(0, 12);
            const localJid = `webclient_${clientUUID}@chromadesk.local/chromoting_ftl_chromadesk_client`;
            const remoteJid = `${deviceId}@chromadesk.local/chromoting_ftl_chromadesk_host`;
    ...
    async _handleJingleMessage(message) {
        ...
        // 当另一个客户端（如 Qt ChromaDesk）同时连接同一 Host 时，
        // 其会话消息也会被转发过来，必须丢弃。
