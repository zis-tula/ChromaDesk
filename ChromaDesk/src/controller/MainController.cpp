...
void MainController::onHostProcessStarted()
{
    ...
    // Start chromadesk-skill-host if enabled
    if (core::LocalConfigCenter::instance().skillHostEnabled()) {
        ...
        if (QFile::exists(skillHostPath)) {
            ...
            LOG_WARN("chromadesk-skill-host not found at {}, skill features disabled",
...
void MainController::startMcpHttpProcess() {
    ...
    if (!QFileInfo::exists(mcpPath)) {
        LOG_ERROR("chromadesk-mcp binary not found: {}",
                  mcpPath.toStdString());
        return;
    }
    ...
    if (!m_mcpHttpProcess) {
        ...
        connect(m_mcpHttpProcess, &QProcess::started,
                this, [this]() {
            LOG_INFO("chromadesk-mcp HTTP process started");
            emit mcpServiceRunningChanged();
        });
        connect(m_mcpHttpProcess,
                QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished),
                this, [this](int exitCode, QProcess::ExitStatus status) {
            LOG_INFO("chromadesk-mcp HTTP process exited: code={}, status={}",
        ...
        connect(m_mcpHttpProcess, &QProcess::errorOccurred,
            ...
            LOG_ERROR("chromadesk-mcp HTTP process error: {}",
    ...
    LOG_INFO("Starting chromadesk-mcp HTTP: {} {}",
             mcpPath.toStdString(),
...
void MainController::stopMcpHttpProcess() {
    ...
    LOG_INFO("Stopping chromadesk-mcp HTTP process");
    ...
    QTimer::singleShot(1000, proc, [proc]() {
        ...
        LOG_WARN("chromadesk-mcp did not exit gracefully, terminating...");
        proc->terminate();
        ...
        QTimer::singleShot(1000, proc, [proc]() {
            if (!proc) return;
            if (proc->state() == QProcess::NotRunning) return;
            LOG_WARN("chromadesk-mcp did not terminate, killing...");
            proc->kill();
        });
    });
}
...
#ifdef Q_OS_WIN
    auto mcpPath = appDir + "/chromadesk-mcp.exe";
#elif defined(Q_OS_MAC)
    auto mcpPath = appDir + "/../../../chromadesk-mcp";
    QFileInfo fileInfo(mcpPath);
    if (!fileInfo.exists() || !fileInfo.isExecutable()) {
        LOG_INFO("using Contents/Frameworks/chromadesk-mcp");   
        // appDir = Contents/MacOS, mcp binary is in Contents/Frameworks
        mcpPath = appDir + "/../Frameworks/chromadesk-mcp";
    } else {
        LOG_INFO("using out of bundle chromadesk-mcp");
    }
#else
    auto mcpPath = appDir + "/chromadesk-mcp";
#endif
...
QString MainController::getSkillHostBinaryPath() const {
    auto appDir = QCoreApplication::applicationDirPath();
#ifdef Q_OS_WIN
    auto skillHostPath = appDir + "/chromadesk-skill-host.exe";
#elif defined(Q_OS_MAC)
    auto skillHostPath = appDir + "/../../../chromadesk-skill-host";
    QFileInfo fileInfo(skillHostPath);
    if (!fileInfo.exists() || !fileInfo.isExecutable()) {
        LOG_INFO("using Contents/Frameworks/chromadesk-skill-host");
        skillHostPath = appDir + "/../Frameworks/chromadesk-skill-host";
    } else {
        LOG_INFO("using out of bundle chromadesk-skill-host");
    }
#else
    auto skillHostPath = appDir + "/chromadesk-skill-host";
#endif
...
QString MainController::getBuiltinSkillsDir() const {
    ...
    // chromadesk-skill-host binary). Falls back to the packaged location
...
QString MainController::generateMcpConfig(const QString& clientType) const {
    ...
    QJsonObject servers;
    servers["chromadesk"] = serverConfig;
